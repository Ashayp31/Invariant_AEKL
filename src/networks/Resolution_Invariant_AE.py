...
class LatentDecompBlock(nn.Module):
    """
    Latent Decomposition block.
    ...
    def __init__(
        ...
        for i in range(out_channels):
            convolutions.append(nn.Conv3d(in_channels=in_channels, out_channels=1, stride=stride, kernel_size=kernel_size, padding=padding))

        self.latent_decomp = nn.ModuleList(convolutions)

    def forward(self,  x: torch.Tensor, latents_to_use: int):
        output = torch.zeros(x.shape[0], self.out_channels, x.shape[2], x.shape[3], x.shape[4], device=x.device)
        ind_conv_outputs = []
        for conv in self.latent_decomp:
            ind_conv_outputs.append(conv(x))
        for i in range(latents_to_use):
            output[:, i] = ind_conv_outputs[i]
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_dims = spatial_dims
        self.num_channels = num_channels
        self.layers = len(num_channels)-1
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.num_res_blocks = num_res_blocks
        self.norm_num_groups = norm_num_groups
    ...
    def forward(self, x: torch.Tensor, spatial_dims:list) -> torch.Tensor:
        ...
        for block in self.blocks:
            if isinstance(block, ResampleBlock):
                x = block(x, spatial_dims[self.layers-s-1])
                s += 1
            else:
                x = block(x)
        return x
...
class AutoencoderKLResolutionInvariant(nn.Module):
    """
    ...
    def sampling(self, z_mu: torch.Tensor, z_sigma: torch.Tensor, noise_schedule, add_noise=True) -> torch.Tensor:
        """
        ...
        if add_noise:
            for i in range(z_vae.shape[1]):
                eps = torch.randn_like(z_sigma)
                z_vae[:, i] = ((1-noise_schedule) * z_vae[:,i]) + (noise_schedule * eps[:,i])

        return z_vae



    def reconstruct(self, x: torch.Tensor, spatial_dimensions_enc: list, spatial_dimensions_dec: list, latents_to_use: int) -> torch.Tensor:
        """
        ...
        """
        z_mu, _ = self.encode(x, spatial_dimensions_enc, latents_to_use)
        reconstruction = self.decode(z_mu, spatial_dimensions_dec)
    ...
    def forward(self, x: torch.Tensor, spatial_dimensions_enc: list, spatial_dimensions_dec: list, noise_to_add: float, recon: str = None,
                latents_to_use: int = 2, add_noise=True) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        z_mu, z_sigma = self.encode(x, spatial_dimensions_enc)
        z = self.sampling(z_mu, z_sigma, noise_to_add, add_noise)
        reconstruction = self.decode(z, spatial_dimensions_dec)
        return reconstruction, z, z_mu, z_sigma

    def encode_stage_2_inputs(self, x: torch.Tensor, spatial_dimensions_enc: list, noise_to_add: float, add_noise=True) -> torch.Tensor:
        z_mu, z_sigma = self.encode(x, spatial_dimensions_enc)
        z = self.sampling(z_mu, z_sigma, noise_to_add, add_noise)
        return z

    def decode_stage_2_outputs(self, z: torch.Tensor, spatial_dimensions_dec: list) -> torch.Tensor:
        image = self.decode(z, spatial_dimensions_dec)
        return image