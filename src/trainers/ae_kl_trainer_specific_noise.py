import json
import os
import sys
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import torch
import torch.nn as nn
import torch.distributed as dist
from generative.losses.adversarial_loss import PatchAdversarialLoss
from generative.losses.perceptual import PerceptualLoss
from generative.networks.nets import PatchDiscriminator

from src.networks import AutoencoderKLResolutionInvariant

from monai.networks.layers import Act
from torch.nn import L1Loss

# from torch.cuda.amp import GradScaler
from torch.nn.parallel import DistributedDataParallel
from torch.utils.tensorboard import SummaryWriter
from tqdm import tqdm

from src.data.get_train_and_val_dataloader import get_training_data_loader


class AEKLTrainerLatentDecompSpecificNoise:
    def __init__(self, args):

        # initialise DDP if run was launched with torchrun
        if "LOCAL_RANK" in os.environ:
            print("Setting up DDP.")
            self.ddp = True
            # disable logging for processes except 0 on every node
            local_rank = int(os.environ["LOCAL_RANK"])
            if local_rank != 0:
                f = open(os.devnull, "w")
                sys.stdout = sys.stderr = f

            # initialize the distributed training process, every GPU runs in a process
            dist.init_process_group(backend="nccl", init_method="env://")
            self.device = torch.device(f"cuda:{local_rank}")
        else:
            self.ddp = False
            self.device = "cuda:0" if torch.cuda.is_available() else "cpu"
        torch.cuda.set_device(self.device)

        print(f"Arguments: {str(args)}")
        for k, v in vars(args).items():
            print(f"  {k}: {v}")


        self.resolution_to_noise_dict = {0: 0,
                            0.1:     0.064566325692216,
                            0.15:    0.068487351122371,
                            0.2:     0.078105017191952,
                            0.3:     0.088441331427086,
                            0.4:     0.096617040844872,
                            0.6:     0.106673193631745,
                            0.8:     0.111024626451242,
                            1:   0.121119358708608,
                            1.2:     0.14625346873782,
                            1.4:     0.164827609443072,
                            1.6:     0.173792271196886,
                            1.8:     0.184495481627112,
                            2:   0.19673307393304,
                            2.4:     0.22202855756054,
                            2.8:     0.247056394667627,
                            3.2:     0.274479690604352,
                            3.6:     0.288402808332848,
                            4:   0.298671072644964,
                            5:   0.328458560932646,
                            6:   0.394685677641798}


        self.resolution_invariant = args.resolution_invariant == 1

        self.network_type = args.trainer_type
        # set up model
        self.spatial_dimension = args.spatial_dimension
        self.num_latent_channels = args.latent_channels
        self.num_layers = len(args.num_channels) - 1


        ae_args = {
            "spatial_dims": args.spatial_dimension,
            "in_channels": args.in_channels,
            "out_channels": args.out_channels,
            "num_channels": args.num_channels,
            "latent_channels": args.latent_channels,
            "num_res_blocks": args.num_res_blocks,
            "norm_num_groups": args.norm_num_groups,
            "attention_levels": args.attention_levels,

        }

        self.model = AutoencoderKLResolutionInvariant(spatial_dims=args.spatial_dimension,
                                         in_channels=args.in_channels,
                                         out_channels=args.out_channels,
                                         num_channels=args.num_channels,
                                         latent_channels=args.latent_channels,
                                         num_res_blocks=args.num_res_blocks,
                                         norm_num_groups=args.norm_num_groups,
                                         attention_levels=args.attention_levels)

        self.model.to(self.device)
        print(f"{sum(p.numel() for p in self.model.parameters()):,} model parameters")

        self.discriminator = PatchDiscriminator(
            spatial_dims=args.spatial_dimension,
            num_layers_d=3,
            num_channels=64,
            in_channels=args.in_channels,
            out_channels=args.out_channels,
            kernel_size=4,
            activation=(Act.LEAKYRELU, {"negative_slope": 0.2}),
            norm="BATCH",
            bias=False,
            padding=1,
        )
        self.discriminator.to(self.device)

        self.perceptual_loss = PerceptualLoss(spatial_dims=3, network_type="alex", fake_3d_ratio=0.25)
        self.perceptual_loss.to(self.device)


        self.optimizer_g = torch.optim.Adam(
            params=self.model.parameters(), lr=args.learning_rate
        )
        self.optimizer_d = torch.optim.Adam(params=self.discriminator.parameters(), lr=0.000001)

        self.l1_loss = L1Loss()
        self.adv_loss = PatchAdversarialLoss(criterion="least_squares")
        self.perceptual_weight = 10
        self.adversarial_weight = 1
        self.recon_weight = 5
        self.kl_weight = args.kl_weight
        self.latent_loss_weight = args.latent_loss_weight

        self.kl_warmup = bool(args.klwarmup)
        self.adversarial_warmup = bool(args.adversarial_warmup)
        # set up optimizer, loss, checkpoints
        self.run_dir = Path(args.output_dir) / args.model_name
        checkpoint_path = self.run_dir / "checkpoint.pth"
        if checkpoint_path.exists():
            checkpoint = torch.load(checkpoint_path)
            self.start_epoch = checkpoint["epoch"] + 1
            self.global_step = checkpoint["global_step"]
            self.model.load_state_dict(checkpoint["model_state_dict"])
            self.best_loss = checkpoint["best_loss"]
            print(
                f"Resuming training using checkpoint {checkpoint_path} at epoch {self.start_epoch}"
            )
        else:
            self.start_epoch = 0
            self.best_loss = 1000
            self.global_step = 0

        # save vqvae parameters
        self.run_dir.mkdir(exist_ok=True)
        with open(self.run_dir / "ae_kl_config.json", "w") as f:
            json.dump(ae_args, f, indent=4)

        if checkpoint_path.exists():
            self.optimizer_d.load_state_dict(checkpoint["optimizer_d_state_dict"])
            self.optimizer_g.load_state_dict(checkpoint["optimizer_g_state_dict"])

        # wrap the model with DistributedDataParallel module
        if self.ddp:
            self.model = DistributedDataParallel(
                self.model,
                device_ids=[self.device],
                find_unused_parameters=False,
                broadcast_buffers=False,
            )
            self.discriminator = DistributedDataParallel(
                self.discriminator,
                device_ids=[self.device],
                find_unused_parameters=False,
                broadcast_buffers=False,
            )

        if args.quick_test:
            print("Quick test enabled, only running on a single train and eval batch.")
        self.quick_test = args.quick_test
        self.logger_train = SummaryWriter(log_dir=str(self.run_dir / "train"))
        self.logger_val = SummaryWriter(log_dir=str(self.run_dir / "val"))
        self.num_epochs = args.n_epochs
        self.train_loader, self.val_loader = get_training_data_loader(
            batch_size=args.batch_size,
            training_ids=args.training_ids,
            validation_ids=args.validation_ids,
            num_workers=args.num_workers,
            cache_data=bool(args.cache_data),
            is_grayscale=bool(args.is_grayscale),
            spatial_dimension=args.spatial_dimension,
            apply_geometric_aug=bool(args.geometric_augmentations),
            normalise_intensity=bool(args.normalise_intensity)
        )

    def save_checkpoint(self, path, epoch, save_message=None, save_every=None):
        if self.ddp and dist.get_rank() == 0:
            # if DDP save a state dict that can be loaded by non-parallel models
            checkpoint = {
                "epoch": epoch + 1,  # save epoch+1, so we resume on the next epoch
                "global_step": self.global_step,
                "model_state_dict": self.model.module.state_dict(),
                "optimizer_g_state_dict": self.optimizer_g.state_dict(),
                "optimizer_d_state_dict": self.optimizer_d.state_dict(),
                "best_loss": self.best_loss,
            }
            print(save_message)
            torch.save(checkpoint, path)
        if not self.ddp:
            checkpoint = {
                "epoch": epoch + 1,  # save epoch+1, so we resume on the next epoch
                "global_step": self.global_step,
                "model_state_dict": self.model.state_dict(),
                "optimizer_g_state_dict": self.optimizer_g.state_dict(),
                "optimizer_d_state_dict": self.optimizer_d.state_dict(),
                "best_loss": self.best_loss,
            }
            print(save_message)
            torch.save(checkpoint, path)



    def train(self, args):

        for epoch in range(self.start_epoch, self.num_epochs):
            self.model.train()
            epoch_loss = self.train_epoch(epoch)
            if epoch_loss < self.best_loss:
                self.best_loss = epoch_loss

                self.save_checkpoint(
                    self.run_dir / "checkpoint.pth",
                    epoch,
                    save_message=f"Saving checkpoint for model with loss {self.best_loss}",
                )

            if args.checkpoint_every != 0 and (epoch + 1) % args.checkpoint_every == 0:
                self.save_checkpoint(
                    self.run_dir / f"checkpoint_{epoch+1}.pth",
                    epoch,
                    save_message=f"Saving checkpoint at epoch {epoch+1}",
                    save_every=args.checkpoint_every
                )

            if (epoch + 1) % args.eval_freq == 0:
                self.model.eval()
                self.val_epoch(epoch)
        print("Training completed.")
        if self.ddp:
            dist.destroy_process_group()


    def calc_noise_to_add(self, low_resolution):
        resolution_scale = low_resolution
        past_value = 0
        for key, value in self.resolution_to_noise_dict.items():
            if resolution_scale < key:
                return past_value
            else:
                past_value = value
        return 0.4

    def train_epoch(self, epoch):
        progress_bar = tqdm(
            enumerate(self.train_loader),
            total=len(self.train_loader),
            ncols=110,
            position=0,
            leave=True,
        )
        progress_bar.set_description(f"Epoch {epoch}")
        l1_loss = 0
        generator_epoch_loss = 0
        discriminator_epoch_loss = 0
        epoch_step = 0
        self.model.train()
        for step, batch in progress_bar:
            images = batch["image"].to(self.device)
            images_low_res = batch["image_low_res"].to(self.device)

            resolution = batch["resolution"][0].detach().numpy()
            resolution_low_res = batch["resolution_low_res"][0].detach().numpy()
            res_change = ((self.num_layers**2)/resolution[0])**(1/self.num_layers)
            res_change_low_res = ((self.num_layers**2)/resolution_low_res[0])**(1/self.num_layers)
            image_dimensions = images.shape[2:]
            image_dimensions_low_res = images_low_res.shape[2:]

            if self.num_layers == 3:
                intermediate_spatial_dimensions = [[int(image_dimensions[i]) for i in range(self.spatial_dimension)],
                                                    [int(image_dimensions[i]/res_change) for i in range(self.spatial_dimension)],
                                                   [int(image_dimensions[i] / (res_change**2)) for i in range(self.spatial_dimension)],
                                                [int(image_dimensions[i] / (res_change**3)) for i in range(self.spatial_dimension)]]
                intermediate_spatial_dimensions_low_res = [[int(image_dimensions_low_res[i]) for i in range(self.spatial_dimension)],
                                                    [int(image_dimensions_low_res[i]/res_change_low_res) for i in range(self.spatial_dimension)],
                                                   [int(image_dimensions_low_res[i] / (res_change_low_res**2)) for i in range(self.spatial_dimension)],
                                                [int(image_dimensions[i] / (res_change**3)) for i in range(self.spatial_dimension)]]
            elif self.num_layers == 4:
                intermediate_spatial_dimensions = [[int(image_dimensions[i]) for i in range(self.spatial_dimension)],
                                                    [int(image_dimensions[i]/res_change) for i in range(self.spatial_dimension)],
                                                   [int(image_dimensions[i] / (res_change**2)) for i in range(self.spatial_dimension)],
                                                [int(image_dimensions[i] / (res_change**3)) for i in range(self.spatial_dimension)],
                                                   [int(image_dimensions[i] / (res_change**4)) for i in range(self.spatial_dimension)]]
                intermediate_spatial_dimensions_low_res = [[int(image_dimensions_low_res[i]) for i in range(self.spatial_dimension)],
                                                    [int(image_dimensions_low_res[i]/res_change_low_res) for i in range(self.spatial_dimension)],
                                                   [int(image_dimensions_low_res[i] / (res_change_low_res**2)) for i in range(self.spatial_dimension)],
                                                [int(image_dimensions_low_res[i] / (res_change_low_res**3)) for i in range(self.spatial_dimension)],
                                                           [int(image_dimensions[i] / (res_change**4)) for i in range(self.spatial_dimension)]]


            resolution_scale_lr = (resolution_low_res/resolution)-1

            noise_to_add_hr = self.calc_noise_to_add(0)
            noise_to_add_lr = self.calc_noise_to_add(resolution_scale_lr)


            self.optimizer_g.zero_grad(set_to_none=True)

            # Generator part

            reconstruction, z, z_mu, z_sigma = self.model(images, intermediate_spatial_dimensions, intermediate_spatial_dimensions, noise_to_add_hr)
            reconstruction_lr, z_lr, z_mu_lr, z_sigma_lr = self.model(images_low_res, intermediate_spatial_dimensions_low_res, intermediate_spatial_dimensions, noise_to_add_lr)

            # Adversarial Loss Generator
            logits_fake = self.discriminator(reconstruction.contiguous().float())[-1]
            logits_fake_lr = self.discriminator(reconstruction_lr.contiguous().float())[-1]

            adversarial_loss = self.adv_loss(
                logits_fake, target_is_real=True, for_discriminator=False
            )
            adversarial_loss_lr = self.adv_loss(
                logits_fake_lr, target_is_real=True, for_discriminator=False
            )
            total_adv_loss = (adversarial_loss + adversarial_loss_lr)/2

            # Recon Loss
            recons_loss = self.l1_loss(reconstruction.float(), images.float())
            recons_loss_lr = self.l1_loss(reconstruction_lr.float(), images.float())
            total_recon_loss = (recons_loss + recons_loss_lr)/2
            # total_recon_loss = recons_loss

            # Perceptual Loss
            perceptual_loss = self.perceptual_loss(reconstruction.float(), images.float())
            perceptual_loss_lr = self.perceptual_loss(reconstruction_lr.float(), images.float())
            total_perceptual_loss = (perceptual_loss + perceptual_loss_lr)/2
            # total_perceptual_loss = perceptual_loss


            # KL Loss
            kl_loss = 0.5 * torch.sum(z_mu.pow(2) + z_sigma.pow(2) - torch.log(z_sigma.pow(2)) - 1,
                                      dim=[1, 2, 3, 4])
            kl_loss = torch.sum(kl_loss) / kl_loss.shape[0]

            kl_loss_lr = 0.5 * torch.sum(z_mu_lr.pow(2) + z_sigma_lr.pow(2) - torch.log(z_sigma_lr.pow(2)) - 1,
                                      dim=[1, 2, 3, 4])
            kl_loss_lr = torch.sum(kl_loss_lr) / kl_loss_lr.shape[0]
            total_kl_loss = (kl_loss + kl_loss_lr)/2

            if self.kl_warmup:
                kl_weight = self.kl_weight * min(epoch, 50) / 50
            else:
                kl_weight = self.kl_weight

            # Latent Loss
            non_zero_latent_loss = self.l1_loss(z_mu.float(), z_mu_lr.float())

            latent_loss = non_zero_latent_loss


            if self.adversarial_warmup:
                adv_weight = self.adversarial_weight * min(epoch, 50) / 50
            else:
                adv_weight = self.adversarial_weight

            total_generator_loss = (self.recon_weight * total_recon_loss) + (kl_weight * total_kl_loss) + (
                    adv_weight * total_adv_loss) + (self.perceptual_weight * total_perceptual_loss) + (self.latent_loss_weight*latent_loss)


            total_generator_loss.backward()
            self.optimizer_g.step()

            # Discriminator part
            self.optimizer_d.zero_grad(set_to_none=True)

            logits_fake = self.discriminator(reconstruction.contiguous().detach())[-1]
            loss_d_fake = self.adv_loss(logits_fake, target_is_real=False, for_discriminator=True)
            logits_real = self.discriminator(images.contiguous().detach())[-1]
            loss_d_real = self.adv_loss(logits_real, target_is_real=True, for_discriminator=True)
            discriminator_loss = (loss_d_fake + loss_d_real) * 0.5

            loss_d = adv_weight * discriminator_loss

            loss_d.backward()
            self.optimizer_d.step()
            discriminator_epoch_loss += discriminator_loss.item()


            l1_loss += recons_loss.item()
            generator_epoch_loss += total_generator_loss.item()
            epoch_step += images.shape[0]
            self.global_step += images.shape[0]
            progress_bar.set_postfix(
                {
                    "l1_loss": l1_loss / (epoch_step),
                    "generator_loss": generator_epoch_loss / (epoch_step),
                    "discriminator_loss": discriminator_epoch_loss / (epoch_step),
                }
            )

            self.logger_train.add_scalar(
                tag="l1_loss", scalar_value=recons_loss.item(), global_step=self.global_step
            )
            self.logger_train.add_scalar(
                tag="latent_loss_non_zero", scalar_value=non_zero_latent_loss.item(), global_step=self.global_step
            )
            self.logger_train.add_scalar(
                tag="perceptual_loss",
                scalar_value=perceptual_loss.item(),
                global_step=self.global_step,
            )
            self.logger_train.add_scalar(
                tag="kl_loss", scalar_value=kl_loss.item(), global_step=self.global_step
            )
            self.logger_train.add_scalar(
                tag="adversarial_loss",
                scalar_value=adversarial_loss.item(),
                global_step=self.global_step,
            )
            self.logger_train.add_scalar(
                tag="generator_loss",
                scalar_value=total_generator_loss.item(),
                global_step=self.global_step,
            )
            self.logger_train.add_scalar(
                tag="discriminator_loss",
                scalar_value=discriminator_loss.item(),
                global_step=self.global_step,
            )
            if self.quick_test:
                break
        epoch_loss = generator_epoch_loss / epoch_step
        return epoch_loss

    def val_epoch(self, epoch):
        with torch.no_grad():
            progress_bar = tqdm(
                enumerate(self.val_loader),
                total=len(self.val_loader),
                ncols=110,
                position=0,
                leave=True,
                desc="Validation",
            )

            global_val_step = self.global_step
            for step, batch in progress_bar:
                images = batch["image"].to(self.device)
                images_low_res = batch["image_low_res"].to(self.device)

                resolution = batch["resolution"][0].detach().numpy()
                resolution_low_res = batch["resolution_low_res"][0].detach().numpy()
                resolution_tensor = batch["resolution_array"].to(self.device)
                resolution_low_res_tensor = batch["resolution_low_res_array"].to(self.device)
                res_change = ((self.num_layers ** 2) / resolution[0]) ** (1 / self.num_layers)
                res_change_low_res = ((self.num_layers ** 2) / resolution_low_res[0]) ** (1 / self.num_layers)
                image_dimensions = images.shape[2:]
                image_dimensions_low_res = images_low_res.shape[2:]

                if self.num_layers == 3:
                    intermediate_spatial_dimensions = [
                        [int(image_dimensions[i]) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / res_change) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 2)) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 3)) for i in range(self.spatial_dimension)]]
                    intermediate_spatial_dimensions_low_res = [
                        [int(image_dimensions_low_res[i]) for i in range(self.spatial_dimension)],
                        [int(image_dimensions_low_res[i] / res_change_low_res) for i in range(self.spatial_dimension)],
                        [int(image_dimensions_low_res[i] / (res_change_low_res ** 2)) for i in
                         range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 3)) for i in range(self.spatial_dimension)]]
                elif self.num_layers == 4:
                    intermediate_spatial_dimensions = [
                        [int(image_dimensions[i]) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / res_change) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 2)) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 3)) for i in range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 4)) for i in range(self.spatial_dimension)]]
                    intermediate_spatial_dimensions_low_res = [
                        [int(image_dimensions_low_res[i]) for i in range(self.spatial_dimension)],
                        [int(image_dimensions_low_res[i] / res_change_low_res) for i in range(self.spatial_dimension)],
                        [int(image_dimensions_low_res[i] / (res_change_low_res ** 2)) for i in
                         range(self.spatial_dimension)],
                        [int(image_dimensions_low_res[i] / (res_change_low_res ** 3)) for i in
                         range(self.spatial_dimension)],
                        [int(image_dimensions[i] / (res_change ** 4)) for i in range(self.spatial_dimension)]]


                resolution_scale = resolution - 1
                resolution_scale_lr = resolution_low_res - 1
                noise_to_add_hr = self.calc_noise_to_add(resolution_scale)
                noise_to_add_lr = self.calc_noise_to_add(resolution_scale_lr)


                reconstruction, _, z_mu, z_sigma = self.model(images, intermediate_spatial_dimensions,
                                                              intermediate_spatial_dimensions, noise_to_add_hr,
                                                              )

                logits_fake = self.discriminator(reconstruction.contiguous().float())[-1]

                recons_loss = self.l1_loss(reconstruction.float(), images.float())
                perceptual_loss = self.perceptual_loss(reconstruction.float(), images.float())

                adversarial_loss = self.adv_loss(
                    logits_fake, target_is_real=True, for_discriminator=False
                )

                kl_loss = 0.5 * torch.sum(
                    z_mu.pow(2) + z_sigma.pow(2) - torch.log(
                        z_sigma.pow(2)) - 1,
                    dim=[1, 2, 3, 4])
                kl_loss = torch.sum(kl_loss) / kl_loss.shape[0]

                total_generator_loss = (self.recon_weight * recons_loss) + (self.kl_weight * kl_loss) + (
                        self.adversarial_weight * adversarial_loss) + (self.perceptual_weight * perceptual_loss)

                self.logger_val.add_scalar(
                    tag="l1_loss", scalar_value=recons_loss.item(), global_step=global_val_step
                )
                self.logger_val.add_scalar(
                    tag="perceptual_loss", scalar_value=perceptual_loss.item(), global_step=global_val_step
                )
                self.logger_val.add_scalar(
                    tag="kl_loss", scalar_value=kl_loss.item(), global_step=global_val_step
                )

                self.logger_val.add_scalar(
                    tag="generator_loss",
                    scalar_value=total_generator_loss.item(),
                    global_step=global_val_step,
                )

                global_val_step += images.shape[0]

                # plot some recons
                if step == 0:
                    reconstruction_lr_to_hr, _, z_mu_lr, _ = self.model(images_low_res,
                                                                        intermediate_spatial_dimensions_low_res,
                                                                        intermediate_spatial_dimensions, noise_to_add_lr)

                    reconstruction_lr_to_hr_2, _, _, _ = self.model(images_low_res,
                                                                        intermediate_spatial_dimensions_low_res,
                                                                        intermediate_spatial_dimensions,
                                                                        noise_to_add_lr)

                    reconstruction_lr_to_hr_no_noise, _, z_mu_lr, _ = self.model(images_low_res,
                                                                        intermediate_spatial_dimensions_low_res,
                                                                        intermediate_spatial_dimensions,
                                                                        noise_to_add_lr,
                                                                         add_noise=False)


                    reconstruction_lr, _, _, _ = self.model(images_low_res, intermediate_spatial_dimensions_low_res,
                                                   intermediate_spatial_dimensions_low_res,  noise_to_add_lr)


                    fig, axs = plt.subplots(2, 3, figsize=(10, 10))

                    sl = np.s_[0, 0, :, images.shape[4] // 2, :]
                    sl_lr = np.s_[0, 0, :, images_low_res.shape[4] // 2, :]

                    axs[0][0].imshow(images[sl].cpu(), cmap="gray")
                    axs[0][0].axis('off')
                    axs[0][0].title.set_text('Original')

                    axs[0][1].imshow(reconstruction[sl].cpu(), cmap="gray")
                    axs[0][1].axis('off')
                    axs[0][1].title.set_text('High Res Recon')

                    axs[0][2].imshow(reconstruction_lr_to_hr_2[sl].cpu(), cmap="gray")
                    axs[0][2].axis('off')
                    axs[0][2].title.set_text('LR to HR added Noise')

                    axs[1][0].imshow(reconstruction_lr_to_hr[sl].cpu(), cmap="gray")
                    axs[1][0].axis('off')
                    axs[1][0].title.set_text('LR to HR added Noise 2')

                    axs[1][1].imshow(reconstruction_lr[sl_lr].cpu(), cmap="gray")
                    axs[1][1].axis('off')
                    axs[1][1].title.set_text('Low Res Recon')

                    axs[1][2].imshow(reconstruction_lr_to_hr_no_noise[sl].cpu(), cmap="gray")
                    axs[1][2].axis('off')
                    axs[1][2].title.set_text('LR to HR No Noise')


                    plt.show()
                    self.logger_val.add_figure(
                        tag="recons", figure=fig, global_step=self.global_step
                    )

                    z_mu_latent = z_mu[0].cpu()
                    z_mu_latent_lr = z_mu_lr[0].cpu()
                    z_mu_latent = np.absolute(z_mu_latent)
                    z_mu_latent_lr = np.absolute(z_mu_latent_lr)

                    data = [z_mu_latent[l].flatten() for l in range(self.num_latent_channels)]
                    data_lr = [z_mu_latent_lr[l].flatten() for l in range(self.num_latent_channels)]


                    fig, axs = plt.subplots(2, figsize=(10, 10))
                    # Creating axes instance
                    # Creating plot
                    axs[0].boxplot(data, whis=(0, 100))
                    axs[1].boxplot(data_lr, whis=(0, 100))
                    # show plot
                    plt.show()
                    self.logger_val.add_figure(
                        tag="Latent Usage", figure=fig, global_step=self.global_step
                    )
