...
class TransformerConditionalInferer(Inferer):
    def __init__(self, device: Any, threshold: float, embedding_shape: Any, vqvae_net:VQVAE, vqvae_checkpoint: Any, clip_encoding: bool,
    ...
    def __call__(self, inputs: torch.Tensor, network: TransformerBase, *args: Any, **kwargs: Any):
        ...
        resampled_latent, resample_mask = self.resample(network, input_pet, input_ct, revert_ordering, index_sequence)
    ...
    @torch.no_grad()
    def resample(self, transformer_network, pet_in, ct_in, rev_ordering, ind_sequencing):
        ...
        logits = transformer_network(zs_in, ct_in)
...
class LikelihoodMapInferer(Inferer):
    ...
    def __call__(self, inputs: torch.Tensor, network: TransformerBase, *args: Any, **kwargs: Any):
        ...
        likelihood_map = self.get_likelihood_maps(network, input_pet, input_ct, revert_ordering)

        outputs = {"llmap": likelihood_map}
        return outputs

    @torch.no_grad()
    def get_likelihood_maps(self, network, pet_in, ct_in, revert_ordering):
...
class VQVAETransformerUncertaintyCombinedInferer(TransformerConditionalInferer):
    def __init__(self, device: Any, threshold: float, embedding_shape: Any, vqvae_net:VQVAE, vqvae_checkpoint: Any,
